#Greets to T.O.F. original tool
#RE-coded by DarkSolo [+] DarkSoloNetwork [+] 
import httplib
import socket
import sys
import os
...
try:
    ...
    php = ['php/','admin/','administrator/','admin1/','admin2/','admin3/','admin4/','admin5/','usuarios/','usuario/','administrator/','moderator/','webadmin/','adminarea/','bb-admin/','adminLogin/','admin_area/','panel-administracion/','instadmin/',
...
'adm/index.php','adm.php','affiliate.php','adm_auth.php','memberadmin.php','administratorlogin.php']
    ...
    asp = ['asp/','admin/','administrator/','admin1/','admin2/','admin3/','admin4/','admin5/','moderator/','webadmin/','adminarea/','bb-admin/','adminLogin/','admin_area/','panel-administracion/','instadmin/',
...
'adm/index.brf','adm.brf','affiliate.brf','adm_auth.brf','memberadmin.brf','administratorlogin.brf']
    ...
    try:
        site = raw_input(bcolors.OKGREEN + " Target for Scan? [www.target.com] : " + bcolors.ENDC)
        if site == 'alf':
        	print "\n [+] ALF is not an Alien Life Form [+]\n"
        	site = raw_input(bcolors.OKGREEN + " Target for Scan? [www.target.com] : " + bcolors.ENDC)
        site = site.replace("http://","")
        print (bcolors.WARNING + "\t\n Checking target " + site + "..." + bcolors.ENDC)
        conn = httplib.HTTPConnection(site)
        conn.connect()
        print bcolors.OKGREEN + "\t\n [$] " + site + " is Online." + bcolors.ENDC
    except (httplib.HTTPResponse, socket.error) as Exit:
        raw_input(bcolors.FAIL + "\t\n [!] " + site + " offline or invalid URL" + bcolors.ENDC)
        exit()
    print bcolors.CYAN + "\nEnter site source code:" + bcolors.ENDC
    print bcolors.CYAN + " 1 - " + bcolors.ENDC + "PHP"
    print bcolors.CYAN + " 2 - " + bcolors.ENDC + "ASP"
    print bcolors.CYAN + " 3 - " + bcolors.ENDC + "CFM"
    print bcolors.CYAN + " 4 - " + bcolors.ENDC + "JS"
    print bcolors.CYAN + " 5 - " + bcolors.ENDC + "CGI"
    print bcolors.CYAN + " 6 - " + bcolors.ENDC + "BRF"
    print bcolors.OKGREEN + "\nSelect and press ENTER\n" + bcolors.ENDC
    code=input(bcolors.CYAN + "alf > " + bcolors.ENDC)
    if code == 1:
    	read = php
    elif code == 2:
    	read = asp
    ...
    print(bcolors.WARNING + "\t [+] Scanning " + site + "...\n\n" + bcolors.ENDC)
    for admin in read:
        admin = admin.replace("\n","")
        admin = "/" + admin
        host = site + admin
        print (bcolors.OKBLUE + "\t [#] Checking " + bcolors.ENDC + bcolors.WARNING + host + "..." + bcolors.ENDC)
        ...
        if response.status == 200:
            var1 = var1 + 1
            print "%s %s" % (bcolors.OKGREEN + "\n\n>>> " + host, "Admin page found!" + bcolors.ENDC)
            founds.append(host)
            print ""
            quit = raw_input(bcolors.CYAN + "Press enter to continue scanning or 'quit' to exit.\n" + bcolors.ENDC)
            if quit =='quit':
            	exit()
        elif response.status == 404:
            var2 = var2
        elif response.status == 302:
            print "%s %s" % (bcolors.WARNING + "\n>>>" + host, "Possible admin page (302 - Redirect)" + bcolors.ENDC)
        else:
            print "%s %s %s" % (host, " Interesting response:", response.status)
        connection.close()
    print(bcolors.OKGREEN + "\n\n [+] Completed !!! [+]\n" + bcolors.ENDC)
    if var1 == 0:
    	print bcolors.FAIL, var1, " Admin pages found" + bcolors.ENDC
    	print bcolors.OKBLUE, var2, " Total pages scanned" + bcolors.ENDC
    	raw_input("\n Bye Bye")
    else:
    	print bcolors.OKGREEN, var1, " Admin pages found" + bcolors.ENDC
    	print "\n ", founds
    	print bcolors.OKBLUE, "\n ", var2, " Total pages scanned" + bcolors.ENDC
    	raw_input("\n Bye Bye")
except (httplib.HTTPResponse, socket.error):
    print bcolors.FAIL + "\n\t[!] Session Cancelled; Error occured." + bcolors.ENDC
except (KeyboardInterrupt, SystemExit):
    print bcolors.FAIL + "\n\t[!] Session aborted by user" + bcolors.ENDC
